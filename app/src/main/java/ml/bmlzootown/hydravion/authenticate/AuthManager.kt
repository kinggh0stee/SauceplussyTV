...
/**
 * Manages the Sauce+ session credential.
 *
 * Sauce+ is a white-label Floatplane instance that uses cookie-session auth (not
 * OAuth/OIDC tokens). The credential is the `Cookie` header value harvested from
 * the WebView login ([WebLoginActivity]) — typically `sails.sid=...; cf_clearance=...`.
